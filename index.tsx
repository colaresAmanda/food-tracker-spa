          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {library.map((item: any) => (
                <button key={item.id} onClick={() => toggle(item.id)} className={`p-3 rounded-xl border-2 text-xs font-bold transition-all ${selectedIds.has(item.id) ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-gray-100 text-gray-600'}`}>{item.name}</button>
              ))}
            </div>
          </div>
        </div>
...
const StackedAreaTrend = ({ data }: { data: any[] }) => {
  const width = 1000;
  const height = 400;
  const padding = 40;
  const periods = ['night', 'evening', 'afternoon', 'morning'];
  const colors: Record<string, string> = {
    morning: '#fcd34d', // Amber
    afternoon: '#10b981', // Emerald
    evening: '#60a5fa', // Blue
    night: '#6366f1' // Indigo
  };

  const maxValue = Math.max(...data.map(d => periods.reduce((acc, p) => acc + d[p], 0)), 1);

  const getPoints = (pIdx: number) => {
    const subset = periods.slice(0, pIdx + 1);
    const topPoints = data.map((d, i) => {
      const x = (i / (data.length - 1)) * (width - padding * 2) + padding;
      const sum = subset.reduce((acc, p) => acc + d[p], 0);
      const y = height - (sum / maxValue) * (height - padding * 2) - padding;
      return `${x},${y}`;
    });

    const bottomPoints = pIdx === 0 
      ? [`${width - padding},${height - padding}`, `${padding},${height - padding}`]
      : data.map((d, i) => {
          const revIdx = data.length - 1 - i;
          const x = (revIdx / (data.length - 1)) * (width - padding * 2) + padding;
          const sum = periods.slice(0, pIdx).reduce((acc, p) => acc + data[revIdx][p], 0);
          const y = height - (sum / maxValue) * (height - padding * 2) - padding;
          return `${x},${y}`;
        }).reverse();
  ...
  return (
    <div className="relative w-full aspect-video bg-gray-50 rounded-3xl p-4 border border-gray-100 overflow-hidden shadow-inner">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full drop-shadow-sm">
        ...
        {[0.25, 0.5, 0.75].map(v => (
          <line key={v} x1={padding} y1={height - padding - (v * (height - padding * 2))} x2={width - padding} y2={height - padding - (v * (height - padding * 2))} stroke="white" strokeWidth="1" strokeOpacity="0.3" />
        ))}
      </svg>
...
const StatsView = ({ history, library }: any) => {
  const stats = useMemo(() => {
    ...
    library.forEach((l: any) => libMap.set(l.id, l.name));
    ...
    <div className="space-y-8 pb-24 animate-fadeIn">
      <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        ...
        <div className="flex justify-between mt-4 px-4">
          {stats.last7Days.map(d => <span key={d.date} className="text-[9px] font-black text-gray-400 uppercase">{d.label}</span>)}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 pb-24 animate-fadeIn">
      ...
      <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden divide-y divide-gray-50">
        {library.length === 0 ? <p className="p-12 text-center text-gray-300 italic text-sm">Library is empty.</p> : library.map((item: any) => (
...
const App = () => {
  ...
  const handleExport = async () => {
    const data = JSON.stringify({ library, history }, null, 2);
      ...
      <header className="px-6 py-6 bg-white border-b border-gray-100 flex justify-between items-center z-40">
        <div>
          ...
          <div className="flex gap-4 mt-2">
            <button onClick={handleExport} className="text-[10px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-1.5"><i className="fa-solid fa-file-export text-xs"></i> Export</button>
            <button onClick={handleImport} className="text-[10px] font-black text-blue-500 uppercase tracking-widest flex items-center gap-1.5"><i className="fa-solid fa-file-import text-xs"></i> Import</button>
          </div>
        </div>
